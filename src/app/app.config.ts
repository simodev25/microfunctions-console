...
const hostApi = environment.apiUrl;
const portApi = environment.portUrl   ;
const baseURL = `${hostApi}`+ (portApi ? `:${portApi}` : '');
const baseURLApi = `${baseURL}${environment.pathUrl}`;
console.log(baseURLApi)
@Injectable()
export class AppConfig {
  config = {
    name: 'MicroFunctions',
    title: 'MicroFunctions',
    version: '1.0.0',
    isBackend: false,
    hostApi,
    portApi,
